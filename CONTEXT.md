...
Cross-harness agent install kit: CLI runners and the post-grill cook pipeline.
...
**Cook**:
The `let-them-cook` pipeline skill — post-grill orchestration across stages (handoff → pre-review → gate → impl → post-review → optional fix).
...
**Human gate**:
Optional pause after PRE_REVIEW before IMPLEMENTATION. Taken only when PRE_REVIEW ends handoff and stdout with `GATE: REVIEW`; `GATE: CONTINUE` means proceed with no pause. Parent never invents a gate. Not a `RESUME=` value — after a `REVIEW` stop, the human continues the same cook in chat.
